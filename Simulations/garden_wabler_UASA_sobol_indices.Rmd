---
title: "garden_wabler_UA_SA_sobol_indices"
...
```{r}
set.seed(2020)
sim_gw_bm <- rgamma(1000, shape = alpha, rate = beta)
plot(density(sb_data$Mass, na.rm = TRUE))
lines(density(sim_gw_bm), col = "blue")
#dev.off()
```
...
plot(density(gw_ws, na.rm = TRUE))
lines(density(sim_gw_ws), col = "blue")
#dev.off()
```
...
```{r}
plot(x = sim_gw_ws, y = sim_gm_wa, pch = 16)
```


```{r}
plot(x = sim_gamma, y = sim_gm_wa, pch = 16)
```
...
variance. And upto 50 to 60 percent of as fat fraction.
Uncertainty analysis is straight-forwad, quantified using mean, variance and 
quantiles.
