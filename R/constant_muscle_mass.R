...
.constant_muscle_mass <- function(data, constant, speed_control) {
  ...
  simResults <- list(
    bm = data.frame(row.names = id),
    fm = data.frame(row.names = id),
    dist = data.frame(row.names = id),
    deltaM = data.frame(row.names = id),
    E = data.frame(row.names = id),
    minSpeed = data.frame(row.names = id),
    trueSpeed = data.frame(row.names = id),
    rvvmp = data.frame(row.names = id)
  )
  ...
  if (speed_control == "constant_speed") {
    ...
    for (i in 1:nrow(data)) {
      ...
      while (currentFM > 0.00001) {
        ...
        # update body
        simResults$deltaM[i, j] <- (simResults$E[i, j] / cons$energy) * 360
