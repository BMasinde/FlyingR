...
the theory behind bird flight. However, there are two drawbacks whih the program.
First, it is only available for windows, and second, it requires manuall imputation
of bird measurements which is a tedious process when one has thousands of birds to 
analyse. Thus, the aim of this project is to implement these methods in R. R runs on
all major operating system and the package is able to read files from a path, with
multiple bird samples.
